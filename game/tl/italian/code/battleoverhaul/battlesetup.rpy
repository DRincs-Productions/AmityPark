# TODO: Translation updated at 2022-07-19 21:03

# game/code/battleoverhaul/battlesetup.rpy:2
translate italian testbattles_44b3ea96:

    # epa "Okay testing time for the battles of episode 7. First battle is against the Ghost Cobra from Kate's route."
    epa "Ok, è il momento dei test per le battaglie dell'episodio 7. La prima battaglia è contro il Cobra fantasma del percorso di Kate."

# game/code/battleoverhaul/battlesetup.rpy:3
translate italian testbattles_4427ec62:

    # epa "Okay time to be crushed by Ember."
    epa "È ora di farsi schiacciare da Ember."

# game/code/battleoverhaul/battlesetup.rpy:5
translate italian testbattles_aa043cb9:

    # epa "That smarts... well time to pick your route."
    epa "Questo è intelligente... è ora di scegliere il percorso."

# game/code/battleoverhaul/battlesetup.rpy:7
translate italian testbattles_f4c6574d:

    # epa "Okay back to the main quest."
    epa "Torniamo alla missione principale."

# game/code/battleoverhaul/battlesetup.rpy:8
translate italian testbattles_2e535c4e:

    # epa "Rescue Sam from this chump goon of Ember's."
    epa "Salvate Sam da questo idiota di Ember."

# game/code/battleoverhaul/battlesetup.rpy:14
translate italian testbattles_9cce09b2:

    # epa "Here's Ember after you have your second charge."
    epa "Ecco Ember dopo la seconda carica."

# game/code/battleoverhaul/battlesetup.rpy:56
translate italian test_MaddieRoute_5d9dee6b:

    # epa "Oh no! The princess is raging again... prepare to face a big cranky dragon!"
    epa "Oh no! La principessa è di nuovo furiosa... preparatevi ad affrontare un grosso drago irascibile!"

# game/code/battleoverhaul/battlesetup.rpy:62
translate italian test_MaddieRoute_2e1a78ee:

    # epa "(Congrats you are now level 7!)"
    epa "(Congratulazioni per il livello 7!)"

# game/code/battleoverhaul/battlesetup.rpy:73
translate italian test_StarRoute_dc68ccb5:

    # epa "Ectopus and his minions."
    epa "Ectopus e i suoi scagnozzi."

# game/code/battleoverhaul/battlesetup.rpy:79
translate italian test_StarRoute_2e1a78ee:

    # epa "(Congrats you are now level 7!)"
    epa "(Congratulazioni per il livello 7!)"

# game/code/battleoverhaul/battlesetup.rpy:88
translate italian test_PaulinaRoute_00abc948:

    # epa "The return of Vlad's monster."
    epa "Il ritorno del mostro di Vlad."

# game/code/battleoverhaul/battlesetup.rpy:94
translate italian test_PaulinaRoute_2e1a78ee:

    # epa "(Congrats you are now level 7!)"
    epa "(Congratulazioni per il livello 7!)"

# game/code/battleoverhaul/battlesetup.rpy:103
translate italian test_KateRoute_500257c6:

    # epa "Kate did not have an extra battle."
    epa "Kate non ha avuto una battaglia in più."

# game/code/battleoverhaul/battlesetup.rpy:109
translate italian test_DesireeRoute_877feb45:

    # epa "Lake Monster unavailable currently."
    epa "Il Mostro del Lago non è attualmente disponibile."

translate italian strings:

    # game/code/battleoverhaul/battlesetup.rpy:19
    old "Choose which route you will take"
    new "Scegliere il percorso da seguire"

    # game/code/battleoverhaul/battlesetup.rpy:19
    old "Maddie"
    new "Maddie"

    # game/code/battleoverhaul/battlesetup.rpy:19
    old "Star"
    new "Stella"

    # game/code/battleoverhaul/battlesetup.rpy:19
    old "Paulina"
    new "Paulina"

    # game/code/battleoverhaul/battlesetup.rpy:19
    old "Kate"
    new "Kate"

